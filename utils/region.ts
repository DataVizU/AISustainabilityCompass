...
export const regionListCn = Object.values(regions);

export const regionIndication = {
  北京: {
    economy: 4.5,
    government: 4.0,
    infrastructure: 4.6,
  ...
  provinces: [
    ...
    },
    {
      ...
      cities: [
        ...
        { n: "吉林市", g: "126.564544,43.871988|12" },
...
export const getLocation = () => {
  const regionLocation: Record<string, number[]> = {};
  locations.municipalities.forEach((municipality) => {
    regionLocation[municipality.n] = municipality.g
      .split("|")[0]
      .split(",")
      .map((item) => parseFloat(item));
  });
  locations.provinces.forEach((province) => {
    regionLocation[province.n] = province.g
      .split("|")[0]
      .split(",")
      .map((item) => parseFloat(item));
    province.cities.forEach((city) => {
      regionLocation[city.n] = city.g
        .split("|")[0]
        .split(",")
        .map((item) => parseFloat(item));
    });
  });
  locations.other.forEach((other) => {
    regionLocation[other.n] = other.g
      .split("|")[0]
      .split(",")
      .map((item) => parseFloat(item));
  });
...
export const regionLocation = getLocation();
