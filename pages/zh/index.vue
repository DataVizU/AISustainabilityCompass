...
import { regionIndication, regionListCn, regions } from "~/utils/region";
...
const selectedRegionValue = computed(() => {
  return regionIndication[
    selectedRegion.value as keyof typeof regionIndication
  ];
});
...
<template>
  <div id="background">
    ...
    <div class="absolute top-24 left-14 z-10 flex flex-col gap-24 select-none">
      <div>
        <h1 class="text-3xl text-black font-bold">AI大语言模型</h1>
        <h1 class="text-3xl text-black font-bold mt-1">准备程度评估</h1>
        <h2 class="text-black font-light text-2xl mt-1">你的地区</h2>
      ...
      <div class="w-64 flex flex-col gap-10 font-light">
        ...
        >
          <div
            class="font-bold w-full h-10 flex flex-row gap-4 items-center px-4 rounded-md hover:bg-linear-gradient"
          ...
          >
            <img
              ...
              alt="Total score"
            />
            <div class="flex-1">总分</div>
            <div class="flex-none">{{ selectedRegionValue.all }}</div>
          </div>
          <div
            class="font-bold w-full flex flex-col gap-1 pt-2"
